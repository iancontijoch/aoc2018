...
def compute(s: str) -> int:
    ...
    max_sleep_mins: dict[int, tuple[int, int]] = {}
    for guard, rngs in sleep_ranges.items():
        mins_days = defaultdict(set)
        for rng_day, rng in rngs:
            for rng_minute in rng:
                mins_days[rng_minute].add(rng_day)
        sleepiest_minute = sorted(
            mins_days.items(),
            key=lambda x: -len(x[1]),
        )[0][0]
        max_sleep_mins[guard] = (
            sleepiest_minute,
            len(mins_days[sleepiest_minute]),
        )

    g, (t, _) = max(max_sleep_mins.items(), key=lambda x: x[1][1])
    return g * t
